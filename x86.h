  ...
  inline void set_immediate(T val) {
    ...
    if (imm_size > 8) {
      throw std::logic_error(
          "immediate instruction size may not be more than 8 bytes");
    }
...
static inst push(x86::reg r1) {
  inst i;
  i.set_rex(false);
  i.set_opcode(0x50 + r1);
  return i;
}
static inst pop(x86::reg r1) {
  inst i;
  i.set_rex(false);
...
static inst add(x86::reg r1, x86::reg r2) {
  inst i;
  i.set_rexw(true);
  i.set_opcode(0x01);
...
static inst add(x86::reg dest, int32_t num) {
  inst i;
...
// TODO: implement MOV
static inst mov(x86::reg r1, x86::reg r2) {
  inst i;
  i.set_rexw(true);
  i.set_opcode(0x89);
  i.set_modrm(true, r2, r1);
 ...
 public:
  ...
  static inline int dump_bytes(uint8_t *code, size_t len) {
    ...
    for (int i = 0; i < count; i++) {
      if (insn[i].size > maxsize) maxsize = insn[i].size;
    }
